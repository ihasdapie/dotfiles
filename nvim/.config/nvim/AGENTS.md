...
- **`stdpath("data")`/`stdpath("state")` always prefer fast local disk over $HOME.** `init.lua` step 0 sets `XDG_DATA_HOME`/`XDG_STATE_HOME` to `/scratch/nvim/{data,state}` when `/scratch` exists (bean dev boxes: `$HOME` is NFS there, ~28x slower for metadata-heavy ops — `docs/decisions/0010-node-local-nvme-scratch.md`), else `/tmp/nvim/{data,state}` (any other host — `/tmp` is effectively universal). `legacy.vim` mirrors the same priority for `undodir`/`viewdir`/`directory` (swap), which nvim core computes as defaults *before* init.lua runs and so don't pick up the env change on their own. Only applies if `XDG_DATA_HOME` isn't already set externally (a host-specific override wins). `stdpath("config")` is untouched — config stays under `$HOME`/dotfiles regardless of host.
- **First time on a host with existing $HOME-resident nvim state**, run `scripts/migrate-state-to-scratch.sh` once to carry over installed plugins/mason/venv and — importantly — persistent undo/view history (not regenerable) onto the new location. It's `rsync`-based and additive; the old `$HOME` copies are left in place. Re-run after a cross-node reschedule if `/scratch` comes back cold.
